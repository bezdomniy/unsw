...
def getJsonContent(indicator_id):
    url = 'http://api.worldbank.org/v2/countries/all/indicators/' + \
        indicator_id + '?per_page=2000&date=2013:2018&format=json'
    ...
    try:
        if indicator_id != 'favicon.ico' and jsonContent[0]['pages'] > 1:
            ...
            for page in range(2, pages + 1):
                url = 'http://api.worldbank.org/v2/countries/all/indicators/' + \
                    indicator_id + \
                    '?per_page=2000&date=2013:2018&format=json&page=' + \
                    str(page)
                response = requests.get(url)
                jsonContent = json.load(io.BytesIO(response.content))
        return jsonContent
    except KeyError:
        abort(400, message="Indicator_id {} not found at World Bank!".format(
            indicator_id))
...
def cleanJSON(jsonContent):
    ...
    for item in jsonContent[1]:
        ret['entries'].append(
            {'country': item['country']['value'], 'date': item['date'], 'value': item['value']})
...
def checkIndicatorInResource(location, indicator):
    indicatorExistsCheck = db.session.query(Collection).filter(
        Collection.indicator == indicator, Collection.location == location)
...
class Entry(db.Model):
    collection_id = db.Column(db.String(36), db.ForeignKey(
        'collection.collection_id', ondelete='CASCADE'), primary_key=True)
...
class Collection_ID_Endpoint(Resource):
    ...
    def delete(self, collections, collection_id):
        ...
        query_result = db.session.query(Collection).filter(
            Collection.collection_id == collection_id, Collection.location == collections)
    ...
    def get(self, collections, collection_id):
        ...
        try:
            collections_q = Collection.query.filter(
                Collection.collection_id == collection_id, Collection.location == collections).all()[0].serialize
        except IndexError:
            abort(404, message="ID {} doesn't exist in resource {}".format(
                collection_id, collections))
...
@api.doc(params={'collections': 'Your resource name. Must be alphanumeric, and begin with a letter.', 'collection_id': 'Your collection ID', 'year': '4 character year - 2013-2018', 'country': 'Country name, separated by space or underscore. Space as %20 if using curl.'})
class Country_Year_Endpoint(Resource):
    ...
    def get(self, collections, collection_id, year, country):
        ...
        if not entries:
            abort(
                404, message="No such records found in: /{}/{}".format(collections, collection_id))
...
class Year_Endpoint(Resource):
    ...
    parser.add_argument(
        'q', type=str, help='Optional: topN or bottomN entries by value (up to 999). E.g. top5, bottom1')
